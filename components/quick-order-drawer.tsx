    ...
    <Sheet>
      ...
      <SheetContent className="w-full sm:max-w-md flex flex-col p-0 border-l border-[var(--color-border)] bg-[var(--color-base)] h-full max-h-screen overflow-hidden">
        <div className="p-4 border-b border-[var(--color-border-subtle)] flex-shrink-0">
          <SheetHeader>
            <SheetTitle className="text-left font-display font-bold text-lg text-[var(--color-text)]">
              Quick Order Summary
            </SheetTitle>
            <SheetDescription className="text-left text-xs text-[var(--color-text-muted)]">
          ...
          <div className="py-4 space-y-4">
            <div className="flex gap-4">
              ...
              <div className="flex flex-col justify-center">
                <h3 className="font-semibold text-sm text-[var(--color-text)] leading-tight">{PRODUCT.name}</h3>
                <p className="text-xs text-[var(--color-text-muted)] mt-1">{PRODUCT.subtitle}</p>
              </div>
            </div>

            <div className="space-y-3">
              <h4 className="font-semibold text-[var(--color-text)] text-xs uppercase tracking-wider">What&apos;s Included</h4>
              <ul className="space-y-2.5">
                {BUNDLE_ITEMS.map((section) => (
                  <li key={section.id} className="text-xs">
                    <span className="font-medium text-[var(--color-text)] block mb-0.5">{section.category}</span>
                    <ul className="pl-0 space-y-0.5">
                      {section.items.map((item, idx) => (
                        <li key={idx} className="text-[var(--color-text-muted)] flex items-start">
                          <span className="text-[var(--color-accent)] mr-1.5 mt-0.5">•</span>
        ...
        <div className="p-4 border-t border-[var(--color-border-subtle)] bg-[var(--color-surface)] flex-shrink-0">
